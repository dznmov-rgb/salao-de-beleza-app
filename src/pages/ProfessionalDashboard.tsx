...
export default function ProfessionalDashboard() {
  ...
  useEffect(() => {
    ...
    if (profile?.is_working !== undefined) {
        setIsWorking(profile.is_working || false); // Fallback para false
    }
  }, [user, currentDate]);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <header className="<dyad-problem-report summary="2 problems">
<problem file="src/pages/ProfessionalDashboard.tsx" line="96" column="22" code="2345">Argument of type 'boolean | null' is not assignable to parameter of type 'SetStateAction&lt;boolean&gt;'.
  Type 'null' is not assignable to type 'SetStateAction&lt;boolean&gt;'.</problem>
<problem file="src/components/AddProfessionalModal.tsx" line="27" column="13" code="2554">Expected 4-5 arguments, but got 3.</problem>
</dyad-problem-report><think>**Tackling TypeScript Errors**

I'm working on two TypeScript compile-time errors. The first one is in `ProfessionalDashboard.tsx`, line 96. The `profile.is_working` variable, which can be `boolean | null`, is causing an issue because `setIsWorking` expects a plain boolean. I'm focusing on how to handle the null case and satisfy the type checker.


**Refining Error Resolutions**

I've addressed both TypeScript compile-time errors. For the `ProfessionalDashboard.tsx` issue, I've implemented a null-check to ensure a boolean is always passed to `setIsWorking`. Regarding `AddProfessionalModal.tsx`, I've updated the `signUp` call to include the necessary `role` argument as `'professional'`, and the optional `phone` argument with an empty string. The code should be type-safe and error-free now.


</think>Okay, let's fix these two remaining TypeScript errors.

1.  **`src/pages/ProfessionalDashboard.tsx`**: The `profile.is_working` can be `null`, but `setIsWorking` expects a `boolean`. I'll add a fallback to `false` if `is_working` is `null`.
2.  **`src/components/AddProfessionalModal.tsx`**: The `signUp` function requires a `role` and an optional `phone` argument. I'll pass `'professional'` as the role and an empty string for the phone.

Here are the updated files:

<dyad-write path="src/pages/ProfessionalDashboard.tsx" description="Corrigindo erro de tipo para 'is_working' com valor fallback.">
import { useState, useEffect } from 'react';
import { LogOut, Scissors, Phone, ArrowRight, ToggleLeft, ToggleRight, CalendarDays } from 'lucide-react';
import { useAuth } from '../contexts/AuthContext';
import { supabase } from '../lib/supabase';
